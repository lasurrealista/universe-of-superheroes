...
export class SuperheroesComponent implements OnInit {
  ...
  list$: Observable<Superhero[]> = this.superheroService.getAll().pipe(
    tap( superheroes => this.superheroesProperties.count = superheroes.length )
  );
  ...
  superheroesProperties: {count: number} = {
    count: 0,
  };
  pageSize: number = 10;
  pageStart: number = 1;
